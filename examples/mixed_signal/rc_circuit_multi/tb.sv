...
parameter real tr_exp = 7.515519e-07 ;
parameter real tf_exp = 7.350237e-06 ;
...
initial begin
   ...
   forever @ ( vout ) begin
     if ( vout > 0.2 && prev_vout <= 0.2 ) begin
       ...
       compare_real_fixed_err ( .expected ( tf_exp ) , .actual ( tf ) , .result ( result ) , .max_err ( 1 * ( 2 / 100.0 ) ) ) ;
       ...
       compare_real_fixed_err ( .expected ( tr_exp ) , .actual ( tr ) , .result ( result ) , .max_err ( 1 * ( 2 / 100.0 ) ) ) ;
       update_test_status ( .result ( tresult ) , .this_result ( result ) ) ;
     end else if ( vout < 0.4 && prev_vout >= 0.4 ) begin
       cross_high= $realtime ( ) ;
       $display ( "Crossing high point at %t" , cross_high ) ;
